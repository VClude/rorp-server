...
RegisterNetEvent('t1ger_cardealer:ChangeCommissionSV')
AddEventHandler('t1ger_cardealer:ChangeCommissionSV', function(currentModel, amount, currentID)	
	MySQL.Async.fetchAll('SELECT * FROM vehicle_display WHERE model=@model',{['@model'] = currentModel},function(DisplayVehicles)
		MySQL.Async.execute("UPDATE vehicle_display SET commission=@commission WHERE id=@id",{['@commission'] = amount, ['@id'] = currentID},function(...)
			TriggerClientEvent('t1ger_cardealer:ChangeCommissionCL', -1)
		end)
	end)
end)

-- Event to edit downpayment:
RegisterNetEvent('t1ger_cardealer:ChangeDownpaymentSV')
AddEventHandler('t1ger_cardealer:ChangeDownpaymentSV', function(currentModel, amount, currentID)	
	MySQL.Async.fetchAll("SELECT * FROM vehicle_display WHERE model=@model",{['@model'] = currentModel},function(DisplayVehicles)
		MySQL.Async.execute("UPDATE vehicle_display SET downpayment=@downpayment WHERE id=@id",{['@downpayment'] = amount, ['@id'] = currentID},function(...)
			TriggerClientEvent('t1ger_cardealer:ChangeDownpaymentCL', -1)
		end)
	end)
end)
			...
			MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.identifier}, function(vehData) 
				...
				for k,v in pairs(vehData) do
					if v.repaytime < 1 and v.finance > 1 then
						...
						TriggerClientEvent("t1ger_cardealer:UpdateCarDealerStockCL",xPlayer.source,vehModel,vehPlate)
					end
				end
			end)
		end
...
-- Fetch Car Dealer Core Data:
ESX.RegisterServerCallback("t1ger_cardealer:FetchDisplayCarData", function(source, cb)
	display = {}
...
ESX.RegisterServerCallback('t1ger_cardealer:GetOwnedVehicleToSell', function(source, cb, plate)
	local xPlayer = ESX.GetPlayerFromId(source)
	local sellCar = {}
...
ESX.RegisterServerCallback('t1ger_cardealer:retrieveJobVehicles', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('t1ger_cardealer:setJobVehicleState')
AddEventHandler('t1ger_cardealer:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[t1ger_dealer] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
