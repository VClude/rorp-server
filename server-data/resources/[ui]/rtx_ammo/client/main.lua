...
RegisterNetEvent('rtx_ammo:useAmmoItem')
AddEventHandler('rtx_ammo:useAmmoItem', function(ammo)
        ...
        if weapon ~= nil then
            ...
            if newAmmo < maxAmmo then
                ...
                if Config.EnableInventoryHUD then
                    TriggerServerEvent('rtx_inventoryhud:updateAmmoCount', weapon, newAmmo)
                end
                SetPedAmmo(playerPed, weapon, newAmmo)
                TriggerServerEvent('rtx_ammo:removeAmmoItem', ammo)
                exports['mythic_notify']:SendAlert('success', 'Zbraň přebita')
            else
                exports['mythic_notify']:SendAlert('error', 'Maximální počet nábojů')
            end
        end
    end
end)
