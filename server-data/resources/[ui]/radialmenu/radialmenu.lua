...
RegisterCommand("submenuEmote", function(source, args, rawCommand)
        -- Wait for next frame jus to be safe
        Citizen.Wait(0)
        ...
        showMenu = true SendNUIMessage({
            type = 'init',
            data = subMenuConfigs["submenuEmote"].data,
            resourceName = GetCurrentResourceName()
        })
        SetCursorLocation(0.5, 0.5)
        SetNuiFocus(true, true)
...
RegisterCommand("submenuKendaraan", function(source, args, rawCommand)
    ...
    showMenu = true SendNUIMessage({
        type = 'init',
        data = subMenuConfigs["submenuKendaraan"].data,
        resourceName = GetCurrentResourceName()
    })
    SetCursorLocation(0.5, 0.5)
    SetNuiFocus(true, true)
...
RegisterCommand("subVehicles", function(source, args, rawCommand)
    -- Wait for next frame jus to be safe
    Citizen.Wait(0)
    ...
    showMenu = true SendNUIMessage({
        type = 'init',
        data = subMenuConfigs["subVehicles"].data,
        resourceName = GetCurrentResourceName()
    })
    SetCursorLocation(0.5, 0.5)
    SetNuiFocus(true, true)
