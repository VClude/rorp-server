...
function closeInventory() {
    InventoryLog('Zavirám');
    EndDragging();
	$('.near-players-wrapper').fadeOut();
    $.post("http://rtx_inventoryhud/NUIFocusOff", JSON.stringify({}));
    $('#search').val('');
}
...
function inventorySetup(invOwner, items, money, invTier) {
    ...
    setupPlayerSlots();    
    $('#player-inv-label').html(firstTier.label + '&ensp;(' + invOwner + ')');
    $('#player-inv-id').html('<i class="fad fa-weight-hanging"style="color:#ff66ff"></i>&ensp;' + (playerweight).toFixed(1) + '/' + (firstTier.maxweight).toFixed(1) + ' kg');
...
function secondInventorySetup(invOwner, items, invTier, money) {
    ...
    $('#other-inv-label').html(secondTier.label + '&ensp;(' + invOwner + ')');
    $('#other-inv-id').html('<i class="fad fa-weight-hanging"style="color:#ff66ff"></i>&ensp;' + (secondweight).toFixed(1) + '/' + (secondTier.maxweight).toFixed(1) + ' kg');
    $('#inventoryTwo').data('invOwner', invOwner);
    $('#inventoryTwo').data('invTier', invTier);
    $('#second-title').html(secondTier.label);
    $('#second-cash').html('<i class="fad fa-money-bill-wave"style="color:#ff66ff"></i>&ensp;$' + formatCurrency(money.cash));
    $('#second-black_money').html('<i class="fad fa-sack"style="color:#ff66ff"></i>&ensp;$' + formatCurrency(money.black_money));
    secondUsed = 0;
    $.each(items, function(index, item) {
...
$(document).ready(function() {
            ...
            if (origDrag.data('invOwner') == $(this).parent().data('invOwner')) {
                ...
                if ($(this).data('inventory') == 'inventoryOne') {
                    if (firstTier.maxweight >= playerweight + (item.weight * Movecount)) {
                        ...
                        $.post("http://rtx_inventoryhud/WeightError");
                        EndDragging();
                    }
                } else {
    ...
    $('#use').click(function(event, ui) {
        if (dragging) {
            ...
            if (itemData.usable) {
                InventoryLog('Použivám ' + itemData.label + ' a zavirám ' + itemData.closeUi);
                $.post("http://rtx_inventoryhud/UseItem", JSON.stringify({
    ...
    $("#take").mouseenter(function () {
        ...
        $('.near-players-wrapper').find('.popup-body').html('');
        $('.near-players-wrapper').find('.popup-body').html('');
		$('.near-players-wrapper').find('.popup-header-text').html('Take Money');
        $('.near-players-list .popup-body').append(`<div class="cashtake" data-id="cash">Money</div>`);
        $('.near-players-list .popup-body').append(`<div class="cashtake" data-id="black_money">Dirty Money</div>`);
        $('.near-players-wrapper').fadeIn();
        EndDragging();
    });
    ...
    $("#store").mouseenter(function () {
        ...
        $('.near-players-wrapper').find('.popup-body').html('');
        $('.near-players-wrapper').find('.popup-body').html('');
		$('.near-players-wrapper').find('.popup-header-text').html('Save Money');
        $('.near-players-list .popup-body').append(`<div class="cashstore" data-id="cash">Money</div>`);
        $('.near-players-list .popup-body').append(`<div class="cashstore" data-id="black_money">Dirty Money</div>`);
        $('.near-players-wrapper').fadeIn();
        EndDragging();
    });
    ...
    $('#give').click(function (event, ui) {
        if (draggingItem != null && dragging) {
			...
			$('.near-players-wrapper').find('.popup-header-text').html('Players near you');
			
            if (itemData.canRemove) {
                $.post("http://rtx_inventoryhud/GetNearPlayers", JSON.stringify({
                    originItem: itemData,
                    action: 'give'
                }));
    ...
    $("#pay").mouseenter(function () {
        ...
        successAudio.play();
        $('.near-players-wrapper').find('.popup-body').html('');
		$('.near-players-wrapper').find('.popup-header-text').html('Pay');
        $('.near-players-list .popup-body').append(`<div class="cashchoice" data-id="cash">Money</div>`);
        $('.near-players-list .popup-body').append(`<div class="cashchoice" data-id="black_money">Dirty Money</div>`);
        $('.near-players-wrapper').fadeIn();
        EndDragging();
    });
    ...
    $("#seize").mouseenter(function () {
        ...
        InventoryLog('Zabavení hotovosti od ' + destinationOwner);
        $.post("http://rtx_inventoryhud/SeizeCash", JSON.stringify({
    ...
    $("#steal").mouseenter(function () {
        ...
        InventoryLog('Krádež hotovosti od ' + destinationOwner);
        $.post("http://rtx_inventoryhud/StealCash", JSON.stringify({
            target: destinationOwner
        }));
    });
    ...
    $('#drop').click(function (event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                InventoryLog('Vyhození ' + dropCount + ' ' + itemData.label + ' na zem');
                $.post("http://rtx_inventoryhud/DropItem", JSON.stringify({
                    item: itemData,
                    qty: dropCount
                }));
                successAudio.play();
    ...
    $('#inventoryOne, #inventoryTwo').on('mouseenter', '.slot', function() {
        ...
        if (itemData !== undefined) {
            ...
            if (itemData.stackable) {
                $('.tooltip-div').find('.tooltip-uniqueness').html("Maximum (" + itemData.max + ")");
            } else {
            ...
            if (itemData.description !== undefined) {
                ...
                $('.tooltip-div').find('.tooltip-desc').html("This item havent description");
            }

            if (itemData.weight !== undefined) {
                $('.tooltip-div').find('.tooltip-weight').html('Weight :' + itemData.weight);
            } else {
...
$('.popup-body').on('click', '.cashchoice', function () {
	$('.near-players-wrapper').find('.popup-header-text').html('Hráčí ve vaší blízkosti');
    $.post("http://rtx_inventoryhud/GetNearPlayers", JSON.stringify({
        action: 'pay',
...
$('.popup-body').on('click', '.cashstore', function () {
    $.post("http://rtx_inventoryhud/CashStore", JSON.stringify({
        action: 'cashstore',
        item: $(this).data("id"),
...
$('.popup-body').on('click', '.cashtake', function () {
    $.post("http://rtx_inventoryhud/CashTake", JSON.stringify({
        action: 'cashtake',
        item: $(this).data("id"),
        count: parseInt($("#count").val()),
        owner: destinationOwner,
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            InventoryLog('Přesouvám ' + item.qty + ' ' + item.label + ' ' + ' Z ' + origin.data('invOwner') + ' Slot ' + origin.data('slot') + ' Do ' + destination.parent().data('invOwner') + ' Slot ' + item.slot);
            $.post("http://rtx_inventoryhud/MoveToEmpty", JSON.stringify({
            ...
            InventoryLog('Přesouvám ' + moveQty + ' ' + item.label + ' Z ' + origin.data('invOwner') + ' Slot ' + item.slot + ' Do ' + destination.parent().data('invOwner') + ' Slot ' + item2.slot);
            $.post("http://rtx_inventoryhud/EmptySplitStack", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
        ...
        if (result === 1) {
            ...
            InventoryLog("The target inventory owner has not been defined");
        }
    }
}
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                successAudio.play();
                InventoryLog('Přesouvám ' + moveQty + ' ' + originItem.label + ' Z ' + origin.data('invOwner') + ' Slot ' + originItem.slot + ' DO ' + destination.parent().data('invOwner') + ' Slot' + destinationItem.slot);
                $.post("http://rtx_inventoryhud/SplitStack", JSON.stringify({
                    originOwner: origin.parent().data('invOwner'),
                    originTier: origin.parent().data('invTier'),
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    InventoryLog('Vyměnuji ' + originItem.label + ' Z  ' + destination.parent().data('invOwner') + ' Slot ' + originItem.slot + ' S ' + destinationItem.label + ' Z ' + origin.data('invOwner') + ' Slot ' + destinationItem.slot);
                    $.post("http://rtx_inventoryhud/SwapItems", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                    ...
                    successAudio.play();
                    InventoryLog('Spojuji ' + originItem.label + ' Z ' + origin.data('invOwner') + ' Slot ' + originItem.slot + ' Do ' + destination.parent().data('invOwner') + ' Slot' + destinationItem.slot);
                    $.post("http://rtx_inventoryhud/CombineStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    InventoryLog('Doplňuji ' + originItem.label + ' Do existujicího stacku ' + destination.parent().data('invOwner') + ' Slot ' + destinationItem.slot);
                    $.post("http://rtx_inventoryhud/TopoffStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    DisplayMoveError(origin, destination, "Stack exceeded maximum number");
                }
            }
        ...
        if (result === 1) {
            ...
            InventoryLog("Vlastník cílového inventáře nebyl definován");
        }
    }
}
...
$('.popup-body').on('click', '.player', function () {
    ...
    if (action === "nearPlayersGive") {
        ...
        if (givingItem != null) {
            ...
            InventoryLog(`Dávám ${count} ${givingItem.label} nejbližšímu hráči s Server ID ${target}`);
            $.post("http://rtx_inventoryhud/GiveItem", JSON.stringify({
                target: target,
        ...
        InventoryLog(`Davám ${count} ${givingItem.label} nejbližšímu hráči s Server ID ${target}`);
        $.post("http://rtx_inventoryhud/GiveCash", JSON.stringify({
            target: target,
            item: givingItem,
            count: count
        }), function (status) {
    ...
    if ($('#action-bar').is(':visible')) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (items[i] != null) {
                ...
                $(`.slot-${i}`).find('.item-name').html('ŽADNÝ');
