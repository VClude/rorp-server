			...
			if not HackingATM then
				for k,v in pairs(Config.ATMs) do
					if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true) <= 1.5 then
						DrawText3Ds(v.x,v.y,v.z, "Tekan ~g~[G]~s~ untuk ~r~Hack~s~ ~y~ATM~s~")
...
function hackingEvent(success)
    ...
    if success then
        ...
        exports['mythic_notify']:DoLongHudText('success','Kamu berhasil hack ATM')
	else
		exports['mythic_notify']:DoLongHudText('error','Kamu gagal hack ATM')
	end
