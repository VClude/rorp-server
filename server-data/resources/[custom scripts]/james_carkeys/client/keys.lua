RegisterNetEvent("james_carkeys:keyTransfered")
AddEventHandler("james_carkeys:keyTransfered", function(keyData)
    table.insert(cachedData["keys"], keyData)

    ESX.ShowNotification("You received a key.", "error", 3500)
end)

RegisterNetEvent("james_carkeys:addKey")
AddEventHandler("james_carkeys:addKey", function(keyData)
    AddKey(keyData)
end)
...
AddKey = function(keyData)
    ...
    ESX.TriggerServerCallback("james_carkeys:addKey", function(added)
        if added then
            table.insert(cachedData["keys"], keyData)
...
RemoveKey = function(keyUUID)
    ...
    for keyIndex, keyData in ipairs(cachedData["keys"]) do
        if keyData["uuid"] == keyUUID then
            ESX.TriggerServerCallback("james_carkeys:removeKey", function(removed)
                if removed then
...
TransferKey = function(keyData, newPlayer)
    ...
    for keyIndex, currentKeyData in ipairs(cachedData["keys"]) do
        if keyData["uuid"] == currentKeyData["uuid"] then
            ESX.TriggerServerCallback("james_carkeys:transferKey", function(removed)
                if removed then
                    table.remove(cachedData["keys"], keyIndex)
