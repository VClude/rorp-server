...
function menuToggle(bool,send=false) {
	...
	if(send){
		$.post('http://rtx_tuning/togglemenu', JSON.stringify({state:false}));
	}
}
...
$(function(){
	...
	$("#boost").slider({min:0.05,value:0.25,step:0.01,max:0.38,change:sliderUpdated});
	$("#fuelmix").slider({min:1,value:1.3,step:0.01,max:3,change:sliderUpdated});
	$("#gearchange").slider({min:1,value:12,max:25,step:2,change:sliderUpdated});
	...
	$("#savebtn").click(function(){
		initiateTyepwriter();
		$.post('http://rtx_tuning/save', JSON.stringify(getSliderValues()));
	});
