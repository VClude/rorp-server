...
------------------------------------------------------------------
ESX = nil
local isPropertymakeupCreatorOpened = false		-- Change this value to show/hide UI
...
RegisterNUICallback('updateSkinSavePropertymakeup', function(data)
	...
	if(v == true) then
		...
		TriggerServerEvent('esx_propertymakeupskin:savePropertymakeup', lipstick, lipstickcolor, lipstickopacity, blush, blushcolor, blushopacity, makeup, makeupcolor, makeupcolor2, makeupopacity)
		
		ClosePropertymakeupCreator()
	else
...
--                          Functions
------------------------------------------------------------------
function ClosePropertymakeupCreator()
	local ped = PlayerPedId()
	isPropertymakeupCreatorOpened = false
	ShowPropertymakeupCreator(false)
...
function ShowPropertymakeupCreator(enable)
local elements    = {}
	...
	SendNUIMessage({
		openPropertymakeupCreator = enable,
	})
...
RegisterNetEvent('hud:loadMenuPropertymakeup')
AddEventHandler('hud:loadMenuPropertymakeup', function()
	ShowPropertymakeupCreator(true)
end)
...
--esx_propertymakeupskin code--
function OpenMenuPropertymakeup(submitCb, cancelCb, restrict)
	local playerPed = PlayerPedId()
	...
	TriggerEvent('skinchanger:getData', function(components, maxVals)
		...
		ESX.UI.MenuPropertymakeup.Open('default', GetCurrentResourceName(), 'skin', {
			title    = _U('skin_MenuPropertymakeup'),
			align    = 'top-left',
			elements = elements
		}, function(data, MenuPropertymakeup)
			TriggerEvent('skinchanger:getSkin', function(skin)
				lastSkinOld = skin
			end)

			submitCb(data, MenuPropertymakeup)
			DeleteSkinCam()
		end, function(data, MenuPropertymakeup)
			MenuPropertymakeup.close()
			DeleteSkinCam()
			TriggerEvent('skinchanger:loadSkin', lastSkinOld)

			if cancelCb ~= nil then
				cancelCb(data, MenuPropertymakeup)
			end
		end, function(data, MenuPropertymakeup)
			...
			if skin[data.current.name] ~= data.current.value then
				...
				for i=1, #elements, 1 do
					...
					MenuPropertymakeup.update({name = elements[i].name}, newData)
				end

				MenuPropertymakeup.refresh()
			end
		end, function(data, MenuPropertymakeup)
			DeleteSkinCam()
		end)
...
function OpenSaveableMenuPropertymakeup(submitCb, cancelCb, restrict)
	TriggerEvent('skinchanger:getSkin', function(skin)
		lastSkinOld = skin
	end)

	OpenMenuPropertymakeup(function(data, MenuPropertymakeup)
		MenuPropertymakeup.close()
		DeleteSkinCam()

		TriggerEvent('skinchanger:getSkin', function(skin)
			TriggerServerEvent('esx_propertymakeupskin:save', skin)
...
AddEventHandler('esx_propertymakeupskin:getLastSkin', function(cb)
	cb(lastSkinOld)
end)

AddEventHandler('esx_propertymakeupskin:setLastSkin', function(skin)
	lastSkinOld = skin
end)

RegisterNetEvent('esx_propertymakeupskin:openMenuPropertymakeup')
AddEventHandler('esx_propertymakeupskin:openMenuPropertymakeup', function(submitCb, cancelCb)
	OpenMenuPropertymakeup(submitCb, cancelCb, nil)
end)

RegisterNetEvent('esx_propertymakeupskin:openRestrictedMenuPropertymakeup')
AddEventHandler('esx_propertymakeupskin:openRestrictedMenuPropertymakeup', function(submitCb, cancelCb, restrict)
	OpenMenuPropertymakeup(submitCb, cancelCb, restrict)
end)

RegisterNetEvent('esx_propertymakeupskin:openSaveableMenuPropertymakeup')
AddEventHandler('esx_propertymakeupskin:openSaveableMenuPropertymakeup', function(submitCb, cancelCb)
	ShowPropertymakeupCreator(true)
end)

RegisterNetEvent('esx_propertymakeupskin:openSaveableRestrictedMenuPropertymakeup')
AddEventHandler('esx_propertymakeupskin:openSaveableRestrictedMenuPropertymakeup', function(submitCb, cancelCb, restrict)
	OpenSaveableMenuPropertymakeup(submitCb, cancelCb, restrict)
end)

RegisterNetEvent('esx_propertymakeupskin:requestSaveSkin')
AddEventHandler('esx_propertymakeupskin:requestSaveSkin', function()
	TriggerEvent('skinchanger:getSkin', function(skin)
		TriggerServerEvent('esx_propertymakeupskin:responseSaveSkin', skin)
	end)
end)
